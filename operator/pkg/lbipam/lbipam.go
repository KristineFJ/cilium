...
func newLBIPAM(params LBIPAMParams) *LBIPAM {
	...
	if params.OperatorConfig.BGPAnnounceLBIP {
...
func (ipam *LBIPAM) Run() {
	...
	err := ipam.init(ipam.ctx, poolChan, svcChan)
	if err != nil {
	...
	// blocking the resources in case they are re-used after we shutdown.
...
func (ipam *LBIPAM) poolOnUpsert(k resource.Key, pool *cilium_api_v2alpha1.CiliumLoadBalancerIPPool) error {
	...
	var err error
	if _, exists := ipam.poolStore.GetByName(pool.GetName()); exists {
	...
	if err != nil {
		if isRetryableError(err) {
			// Return the error, this will trigger the service resource to retry this service at a later time in the hope
			// that any transient error is resolved by then.
			return err
		}
...
func (ipam *LBIPAM) stripInvalidAllocations(sv *ServiceView) error {
	...
	for allocIdx := len(sv.AllocatedIPs) - 1; allocIdx >= 0; allocIdx-- {
		...
		pool, found := ipam.poolStore.GetByName(alloc.Origin.originPool)
		if !found {
			err := releaseAllocIP()
			if err != nil {
...
func (ipam *LBIPAM) findRangeOfIP(sv *ServiceView, ip net.IP) (lbRange *LBRange, foundPool bool, err error) {
	for _, r := range ipam.rangesStore.ranges {
		...
		pool, found := ipam.poolStore.GetByName(r.originPool)
		if !found {
			continue
		}
...
func (ipam *LBIPAM) allocateIPAddress(
	...
	for _, lbRange := range ipam.rangesStore.ranges {
		...
		pool, found := ipam.poolStore.GetByName(lbRange.originPool)
		if !found {
...
func (ipam *LBIPAM) handleNewPool(pool *cilium_api_v2alpha1.CiliumLoadBalancerIPPool) error {
	...
	if _, found := ipam.poolStore.GetByName(pool.GetName()); found {
...
// isRetryableError returns true if the error is likely due to temporary conditions and thus can be retried.
func isRetryableError(err error) bool {
	// This is likely a network related error
	var opErr *net.OpError
	if errors.As(err, &opErr) {
		return true
	}

	// Syscall errors can per permission related but also to due temporary things like resource exhaustion. So lets
	// retry them.
	var syscallErr *os.SyscallError
	if errors.As(err, &syscallErr) {
		return true
	}
	var errorNo syscall.Errno
	if errors.As(err, &errorNo) {
		return true
	}
