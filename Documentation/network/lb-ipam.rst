...
type ``LoadBalancer``. This functionally is usually left up to a cloud provider,
...
LB IPAM is implicitly enabled when cilium is installed with the
``bgpControlPlane.enabled = true`` and ``bgp.announce.loadbalancerIP = true``
helm values.
...
.. code-block:: shell-session
    ...
    $ kubectl get ippools/red-pool -o jsonpath='{.status.conflictReason}'
...
.. code-block:: shell-session
    ...
    Status:
      #[...]
      Conflict Reason:  Pool conflicts since CIDR '20.0.10.0/24' overlaps CIDR '20.0.10.0/24' from IP Pool 'blue-pool'
      Conflicting:      true
      #[...]
...
.. code-block:: shell-session
    ...
    $ kubectl get ippools -o jsonpath='{.items[*].status}' | jq
    {
      "cidrCounts": {
        "20.0.10.0/24": {
          "available": 254,
...
If the ``.spec.loadBalancerClass`` is set to a class which isn't handled by LB IPAM, then LB IPAM
will ignore the service entirely, not even setting a condition in the status. 
